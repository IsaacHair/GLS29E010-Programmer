...
uint16_t prog[LEN][4]={
    ...
    {0x3000, ADR+0, 0x0003, 0x3001}, //if the value is a zero - bit 0
    ...
    {0x3100, ADR+0, 0x1000, 0x3101}, //bit 1
    ...
    {0x3200, ADR+0, 0x1010, 0x3201}, //bit 2
    ...
    {0x3300, ADR+0, 0x1020, 0x3301}, //add on last bit and clear rest of number
    {0x3301, ADR+1, 0x1030, 0x5000},


    {0x4000, ADR+0, 0x0003, 0x4001}, //if the value is a one - bit 0
    ...
    {0x4100, ADR+0, 0x1000, 0x4101}, //bit 1
    ...
    {0x4200, ADR+0, 0x1010, 0x4201}, //bit 2
    ...
    {0x4300, ADR+0, 0x1020, 0x4301}, //set address properly and write last bit of address
    ...
    {0x5006, BUF+1, 0x0000, 0x5200}, //flip to one
    {0x5007, BUF+0, 0x0000, 0x5100}, //flip to zero


    {0x5100, ADR+0, 0x103f, 0x5101}, //printing 0
    {0x5101, ADR+1, 0x0003, 0x5102}, //3rd digit
    {0x5102, JMP+R, 0x0000, 0x5104},
    {0x5104, ADR+0, 0x0003, 0x5110},
    {0x5105, ADR+0, 0x0003, 0x5120},


    {0x5110, ADR+0, 0x0003, 0x5111}, //2nd digit
    {0x5111, ADR+1, 0x0002, 0x5112},
    {0x5112, JMP+R, 0x0000, 0x5114},
    {0x5114, ADR+0, 0x0003, 0x5130},
    {0x5115, ADR+0, 0x0003, 0x5140},

    {0x5120, ADR+0, 0x0003, 0x5121},
    {0x5121, ADR+1, 0x0002, 0x5122},
    {0x5122, JMP+R, 0x0000, 0x5124},
    {0x5124, ADR+0, 0x0003, 0x5150},
    {0x5125, ADR+0, 0x0003, 0x5160},


    {0x5130, ADR+0, 0x0003, 0x5131}, //1st digit
    {0x5131, ADR+1, 0x0001, 0x5132},
    {0x5132, JMP+R, 0x0000, 0x5134},
    {0x5134, ADR+0, 0x0003, 0x5170},
    {0x5135, ADR+0, 0x0003, 0x5180},

    {0x5140, ADR+0, 0x0003, 0x5141},
    {0x5141, ADR+1, 0x0001, 0x5142},
    {0x5142, JMP+R, 0x0000, 0x5144},
    {0x5144, ADR+0, 0x0003, 0x5190},
    {0x5145, ADR+0, 0x0003, 0x51a0},

    {0x5150, ADR+0, 0x0003, 0x5151},
    {0x5151, ADR+1, 0x0001, 0x5152},
    {0x5152, JMP+R, 0x0000, 0x5154},
    {0x5154, ADR+0, 0x0003, 0x51b0},
    {0x5155, ADR+0, 0x0003, 0x51c0},

    {0x5160, ADR+0, 0x0003, 0x5161},
    {0x5161, ADR+1, 0x0001, 0x5162},
    {0x5162, JMP+R, 0x0000, 0x5164},
    {0x5164, ADR+0, 0x0003, 0x51d0},
    {0x5165, ADR+0, 0x0003, 0x51e0},


    {0x5170, ADR+0, 0x0003, 0x5171}, //0th digit
    {0x5171, ADR+1, 0x0000, 0x5172},
    {0x5172, JMP+R, 0x0000, 0x5174},
    {0x5174, ADR+0, 0x0003, 0x5400},
    {0x5175, ADR+0, 0x0003, 0x5401},

    {0x5180, ADR+0, 0x0003, 0x5181},
    {0x5181, ADR+1, 0x0000, 0x5182},
    {0x5182, JMP+R, 0x0000, 0x5184},
    {0x5184, ADR+0, 0x0003, 0x5402},
    {0x5185, ADR+0, 0x0003, 0x5403},

    {0x5190, ADR+0, 0x0003, 0x5191},
    {0x5191, ADR+1, 0x0000, 0x5192},
    {0x5192, JMP+R, 0x0000, 0x5194},
    {0x5194, ADR+0, 0x0003, 0x5404},
    {0x5195, ADR+0, 0x0003, 0x5405},
    
    {0x51a0, ADR+0, 0x0003, 0x51a1},
    {0x51a1, ADR+1, 0x0000, 0x51a2},
    {0x51a2, JMP+R, 0x0000, 0x51a4},
    {0x51a4, ADR+0, 0x0003, 0x5406},
    {0x51a5, ADR+0, 0x0003, 0x5407},

    {0x51b0, ADR+0, 0x0003, 0x51b1}, //still 0th digit
    {0x51b1, ADR+1, 0x0000, 0x51b2},
    {0x51b2, JMP+R, 0x0000, 0x51b4},
    {0x51b4, ADR+0, 0x0003, 0x5408},
    {0x51b5, ADR+0, 0x0003, 0x5409},

    {0x51c0, ADR+0, 0x0003, 0x51c1},
    {0x51c1, ADR+1, 0x0000, 0x51c2},
    {0x51c2, JMP+R, 0x0000, 0x51c4},
    {0x51c4, ADR+0, 0x0003, 0x540a},
    {0x51c5, ADR+0, 0x0003, 0x540b},

    {0x51d0, ADR+0, 0x0003, 0x51d1},
    {0x51d1, ADR+1, 0x0000, 0x51d2},
    {0x51d2, JMP+R, 0x0000, 0x51d4},
    {0x51d4, ADR+0, 0x0003, 0x540c},
    {0x51d5, ADR+0, 0x0003, 0x540d},

    {0x51e0, ADR+0, 0x0003, 0x51e1},
    {0x51e1, ADR+1, 0x0000, 0x51e2},
    {0x51e2, JMP+R, 0x0000, 0x51e4},
    {0x51e4, ADR+0, 0x0003, 0x540e},
    {0x51e5, ADR+0, 0x0003, 0x540f},
    ...
    {0x5200, ADR+0, 0x103f, 0x5201}, //printing 1
    {0x5201, ADR+1, 0x0003, 0x5202}, //3rd digit
    {0x5202, JMP+R, 0x0000, 0x5204},
    {0x5204, ADR+0, 0x0003, 0x5210},
    {0x5205, ADR+0, 0x0003, 0x5220},


    {0x5210, ADR+0, 0x0003, 0x5211}, //2nd digit
    {0x5211, ADR+1, 0x0002, 0x5212},
    {0x5212, JMP+R, 0x0000, 0x5214},
    {0x5214, ADR+0, 0x0003, 0x5230},
    {0x5215, ADR+0, 0x0003, 0x5240},

    {0x5220, ADR+0, 0x0003, 0x5221},
    {0x5221, ADR+1, 0x0002, 0x5222},
    {0x5222, JMP+R, 0x0000, 0x5224},
    {0x5224, ADR+0, 0x0003, 0x5250},
    {0x5225, ADR+0, 0x0003, 0x5260},


    {0x5230, ADR+0, 0x0003, 0x5231}, //1st digit
    {0x5231, ADR+1, 0x0001, 0x5232},
    {0x5232, JMP+R, 0x0000, 0x5234},
    {0x5234, ADR+0, 0x0003, 0x5270},
    {0x5235, ADR+0, 0x0003, 0x5280},

    {0x5240, ADR+0, 0x0003, 0x5241},
    {0x5241, ADR+1, 0x0001, 0x5242},
    {0x5242, JMP+R, 0x0000, 0x5244},
    {0x5244, ADR+0, 0x0003, 0x5290},
    {0x5245, ADR+0, 0x0003, 0x52a0},
    
    {0x5250, ADR+0, 0x0003, 0x5251},
    {0x5251, ADR+1, 0x0001, 0x5252},
    {0x5252, JMP+R, 0x0000, 0x5254},
    {0x5254, ADR+0, 0x0003, 0x52b0},
    {0x5255, ADR+0, 0x0003, 0x52c0},

    {0x5260, ADR+0, 0x0003, 0x5261},
    {0x5261, ADR+1, 0x0001, 0x5262},
    {0x5262, JMP+R, 0x0000, 0x5264},
    {0x5264, ADR+0, 0x0003, 0x52d0},
    {0x5265, ADR+0, 0x0003, 0x52e0},


    {0x5270, ADR+0, 0x0003, 0x5271}, //0th digit
    {0x5271, ADR+1, 0x0000, 0x5272},
    {0x5272, JMP+R, 0x0000, 0x5274},
    {0x5274, ADR+0, 0x0003, 0x5800},
    {0x5275, ADR+0, 0x0003, 0x5801},

    {0x5280, ADR+0, 0x0003, 0x5281},
    {0x5281, ADR+1, 0x0000, 0x5282},
    {0x5282, JMP+R, 0x0000, 0x5284},
    {0x5284, ADR+0, 0x0003, 0x5802},
    {0x5285, ADR+0, 0x0003, 0x5803},

    {0x5290, ADR+0, 0x0003, 0x5291},
    {0x5291, ADR+1, 0x0000, 0x5292},
    {0x5292, JMP+R, 0x0000, 0x5294},
    {0x5294, ADR+0, 0x0003, 0x5804},
    {0x5295, ADR+0, 0x0003, 0x5805},
   
    {0x52a0, ADR+0, 0x0003, 0x52a1},
    {0x52a1, ADR+1, 0x0000, 0x52a2},
    {0x52a2, JMP+R, 0x0000, 0x52a4},
    {0x52a4, ADR+0, 0x0003, 0x5806},
    {0x52a5, ADR+0, 0x0003, 0x5807},

    {0x52b0, ADR+0, 0x0003, 0x52b1}, //still 0th digit
    {0x52b1, ADR+1, 0x0000, 0x52b2},
    {0x52b2, JMP+R, 0x0000, 0x52b4},
    {0x52b4, ADR+0, 0x0003, 0x5808},
    {0x52b5, ADR+0, 0x0003, 0x5809},
    
    {0x52c0, ADR+0, 0x0003, 0x52c1},
    {0x52c1, ADR+1, 0x0000, 0x52c2},
    {0x52c2, JMP+R, 0x0000, 0x52c4},
    {0x52c4, ADR+0, 0x0003, 0x580a},
    {0x52c5, ADR+0, 0x0003, 0x580b},

    {0x52d0, ADR+0, 0x0003, 0x52d1},
    {0x52d1, ADR+1, 0x0000, 0x52d2},
    {0x52d2, JMP+R, 0x0000, 0x52d4},
    {0x52d4, ADR+0, 0x0003, 0x580c},
    {0x52d5, ADR+0, 0x0003, 0x580d},

    {0x52e0, ADR+0, 0x0003, 0x52e1},
    {0x52e1, ADR+1, 0x0000, 0x52e2},
    {0x52e2, JMP+R, 0x0000, 0x52e4},
    {0x52e4, ADR+0, 0x0003, 0x580e},
    {0x52e5, ADR+0, 0x0003, 0x580f},
