...
int check(long pageindex)
{
  long i;
  long pagen;
  for (i = 0; i < 8; i++)
    pinMode(DATA+i, INPUT);
  pagen = prog[pageindex];
  for (i = pageindex+1; prog[i] != -1; i += 2) {
    Serial.print("address:");
      Serial.print(pagen+prog[i]);
      Serial.print(" value:");
      Serial.print(show(pagen+prog[i]));
      Serial.print("expected:");
      Serial.println(prog[i+1]);
    if (prog[i+1] != show(prog[i])) {
      Serial.print("fail:");
      Serial.println(prog[i+1]);
      Serial.println(show(prog[i]));
      for (i = 0; i < 8; i++)
        pinMode(DATA+i, OUTPUT);
      return (1);
    }
