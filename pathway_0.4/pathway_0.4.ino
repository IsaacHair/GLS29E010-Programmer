  #define LEN 444 //********make sure to get length correct
 #define JMP 0
 #define OUT 2
 #define ADR 4
 #define BUF 6
...
uint16_t prog[LEN][4]={
    ...
    {0x1001, ADR+1, 0x0000, 0x1002}, //0x0000 is ones digit; 0x0003 is 8s digit
    ...
    {0x540f, OUT+0, 0x8000, 0x6000}, //goes to loop anyway; has to reset to 0 so that testing can occur; adds and extra 139 extra cycles even when all digits are printed already
    ...
    {0x5800, OUT+1, 0x0001, 0x6000}, //actually print it
    {0x5801, OUT+1, 0x0002, 0x6000},
    {0x5802, OUT+1, 0x0004, 0x6000},
    {0x5803, OUT+1, 0x0008, 0x6000},
    {0x5804, OUT+1, 0x0010, 0x6000},
    {0x5805, OUT+1, 0x0020, 0x6000},
    {0x5806, OUT+1, 0x0040, 0x6000},
    {0x5807, OUT+1, 0x0080, 0x6000},

    {0x5808, OUT+1, 0x0100, 0x6000},
    {0x5809, OUT+1, 0x0200, 0x6000},
    {0x580a, OUT+1, 0x0400, 0x6000},
    {0x580b, OUT+1, 0x0800, 0x6000},
    {0x580c, OUT+1, 0x1000, 0x6000},
    {0x580d, OUT+1, 0x2000, 0x6000},
    {0x580e, OUT+1, 0x4000, 0x6000}, //this set of instructions equalizes the time required to add
    {0x580f, OUT+1, 0x8000, 0x6000}, //goes to loop anyway; has to reset to 0 so that testing can occur; adds and extra 139 extra cycles even when all digits are printed already



    {0x6000, ADR+0, 0xffff, 0x6001}, //reset to 0; all this does is declare a variable to index and waste time
    {0x6001, ADR+1, 0x2000, 0x6002}, //delaying to equalize so that in total each increment takes 2375 instructions
    {0x6002, BUF+0, 0x0000, 0x6003},

    {0x6003, ADR+0, 0xffff, 0x6004},
    {0x6004, ADR+1, 0x2001, 0x6005},
    {0x6005, BUF+0, 0x0000, 0x6006},
    
    {0x6006, ADR+0, 0xffff, 0x6007},
    {0x6007, ADR+1, 0x2002, 0x6008},
    {0x6008, BUF+0, 0x0000, 0x6010},


    {0x6010, OUT+0, 0x0000, 0x6011}, //padding up to 139 instructions
    {0x6011, OUT+0, 0x0000, 0x6012},
    {0x6012, OUT+0, 0x0000, 0x6013},
    {0x6013, OUT+0, 0x0000, 0x6014},
    {0x6014, OUT+0, 0x0000, 0x6015},
    {0x6015, OUT+0, 0x0000, 0x6016},
    {0x6016, OUT+0, 0x0000, 0x6017},
    {0x6017, OUT+0, 0x0000, 0x6018},

    {0x6018, OUT+0, 0x0000, 0x6019},
    {0x6019, OUT+0, 0x0000, 0x601a},
    {0x601a, OUT+0, 0x0000, 0x601b},
    {0x601b, OUT+0, 0x0000, 0x601c},
    {0x601c, OUT+0, 0x0000, 0x601d},
    {0x601d, OUT+0, 0x0000, 0x601e},
    {0x601e, OUT+0, 0x0000, 0x601f},
    {0x601f, OUT+0, 0x0000, 0x6020},

    {0x6020, OUT+0, 0x0000, 0x6021},
    {0x6021, OUT+0, 0x0000, 0x6100},


    {0x6100, ADR+0, 0xffff, 0x6101}, //add 1 to actual total
    {0x6101, ADR+1, 0x2000, 0x6102}, //0x2000 is ones digit; 0x2002 is 4s digit
    {0x6102, JMP+R, 0x0000, 0x6104},
    {0x6104, BUF+1, 0x0000, 0x6200},
    {0x6105, BUF+0, 0x0000, 0x6110},

    {0x6110, ADR+0, 0xffff, 0x6111},
    {0x6111, ADR+1, 0x2001, 0x6112},
    {0x6112, JMP+R, 0x0000, 0x6114},
    {0x6114, BUF+1, 0x0000, 0x6204},
    {0x6115, BUF+0, 0x0000, 0x6120},

    {0x6120, ADR+0, 0xffff, 0x6121},
    {0x6121, ADR+1, 0x2002, 0x6122},
    {0x6122, JMP+R, 0x0000, 0x6124},
    {0x6124, BUF+1, 0x0000, 0x6100},
    {0x6125, BUF+0, 0x0000, 0x6700}, //has done 8 cycles, so it is done and can move on
    

    {0x6200, OUT+1, 0x0000, 0x6201}, //equalize clock cycles spent to add to index
    {0x6201, OUT+1, 0x0000, 0x6202},
    {0x6202, OUT+1, 0x0000, 0x6203},
    {0x6203, OUT+1, 0x0000, 0x6204},
    
    {0x6204, OUT+1, 0x0000, 0x6205},
    {0x6205, OUT+1, 0x0000, 0x6206},
    {0x6206, OUT+1, 0x0000, 0x6207},
    {0x6207, OUT+1, 0x0000, 0x6100},



    {0x6700, ADR+0, 0xffff, 0x6701}, //add 1; delaying to equalize so that in total each increment takes 2236 instructions
    {0x6701, ADR+1, 0x0000, 0x6702}, //0x0000 is ones digit; 0x0003 is 8s digit
    {0x6702, JMP+R, 0x0000, 0x6704},
    {0x6704, BUF+1, 0x0000, 0x6800},
    {0x6705, BUF+0, 0x0000, 0x6710},

    {0x6710, ADR+0, 0xffff, 0x6711},
    {0x6711, ADR+1, 0x0001, 0x6712},
    {0x6712, JMP+R, 0x0000, 0x6714},
    {0x6714, BUF+1, 0x0000, 0x6804},
    {0x6715, BUF+0, 0x0000, 0x6720},

    {0x6720, ADR+0, 0xffff, 0x6721},
    {0x6721, ADR+1, 0x0002, 0x6722},
    {0x6722, JMP+R, 0x0000, 0x6724},
    {0x6724, BUF+1, 0x0000, 0x6808},
    {0x6725, BUF+0, 0x0000, 0x6730},

    {0x6730, ADR+0, 0xffff, 0x6731},
    {0x6731, ADR+1, 0x0003, 0x6732},
    {0x6732, JMP+R, 0x0000, 0x6734},
    {0x6734, BUF+1, 0x0000, 0x6000},
    {0x6735, BUF+0, 0x0000, 0x0000}, //is set to 0 so fully cycled to waste time and can go back to start of entire program


    {0x6800, OUT+1, 0x0000, 0x6801}, //equalize clock cycles spent to add to index
    {0x6801, OUT+1, 0x0000, 0x6802},
    {0x6802, OUT+1, 0x0000, 0x6803},
    {0x6803, OUT+1, 0x0000, 0x6804},
    
    {0x6804, OUT+1, 0x0000, 0x6805},
    {0x6805, OUT+1, 0x0000, 0x6806},
    {0x6806, OUT+1, 0x0000, 0x6807},
    {0x6807, OUT+1, 0x0000, 0x6808},
    
    {0x6808, OUT+1, 0x0000, 0x6809},
    {0x6809, OUT+1, 0x0000, 0x680a},
    {0x680a, OUT+1, 0x0000, 0x680b},
    {0x680b, OUT+1, 0x0000, 0x6000},
    };
