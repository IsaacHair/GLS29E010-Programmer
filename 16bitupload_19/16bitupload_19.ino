 #define LEN 232 //********make sure to get length correct
 #define JMP 0
 #define OUT 2
 #define ADR 4
 #define BUF 6
...
uint16_t prog[LEN][4]={ //ram: 200X-202X = pathway flags; 2040,2050,2060, 2070 = bit select flags; 2080 = carry flag
                        ...
                        //program: 00XX -> reset each incrementation; 1XXX/2XXX -> pathway to number; 3XXX -> increment the number
                        //4XXX -> increment the pointer; 5XXX -> print number
    {0x0000, OUT+0, 0xffff, 0x0001}, //output zeroing
    
    {0x0001, ADR+0, 0xffff, 0x0002}, //reset pointer
    {0x0002, ADR+1, 0x1000, 0x0003},
    {0x0003, BUF+0, 0x0000, 0x0004},
    
    {0x0004, ADR+0, 0xffff, 0x0005},
    {0x0005, ADR+1, 0x1001, 0x0006},
    {0x0006, BUF+0, 0x0000, 0x0007},
    ...
    {0x130a, ADR+1, 0x2030, 0x130b},
    {0x130b, BUF+0, 0x0000, 0x1000},
    {0x130c, ADR+0, 0xfff0, 0x130d},
    {0x130d, ADR+1, 0x0030, 0x130e},
    ...
    {0x3000, JMP+I, 0x0001, 0x3002}, //check for pausing
    {0x3002, OUT+1, 0x0000, 0x3000},
    {0x3003, OUT+1, 0x0000, 0x4000},
    //then increment

    {0x4000, ADR+0, 0xffff, 0x4001},
    {0x4001, ADR+1, 0x1000, 0x4002},
    {0x4002, JMP+R, 0x0000, 0x4004},
    {0x4004, BUF+1, 0x0000, 0x0010},
    {0x4005, BUF+0, 0x0000, 0x4100},

    {0x4100, ADR+0, 0xffff, 0x4101},
    {0x4101, ADR+1, 0x1001, 0x4102},
    {0x4102, JMP+R, 0x0000, 0x4104},
    {0x4104, BUF+1, 0x0000, 0x0010},
    {0x4105, BUF+0, 0x0000, 0x4200},

    {0x4200, ADR+0, 0xffff, 0x4201},
    {0x4201, ADR+1, 0x1002, 0x4202},
    {0x4202, JMP+R, 0x0000, 0x4204},
    {0x4204, BUF+1, 0x0000, 0x0010},
    {0x4205, BUF+0, 0x0000, 0x4300},

    {0x4300, ADR+0, 0xffff, 0x4301},
    {0x4301, ADR+1, 0x1003, 0x4302},
    {0x4302, JMP+R, 0x0000, 0x4304},
    {0x4304, BUF+1, 0x0000, 0x0010},
    {0x4305, BUF+0, 0x0000, 0x0010},
    };
